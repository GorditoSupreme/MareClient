...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(ILogger logger, string uid)
    {
        ...
        return await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            Guid collId;
            PenumbraApiEc errCode = _penumbraCreateNamedTemporaryCollection.Invoke("Aaron", collName, out collId);
            logger.LogTrace("Creating Temp Collection {collName}, GUID: {collId}, ErrorCode: {errCode}", collName, collId, errCode);
            return collId;
